...
exports.generate = asyncHandler(async (req, res) => {
  ...
  try {
    const result = await aiRouter.generate({ moduleKey: 'ai-voice-agent', messages: [{ role: 'system', content: 'You are a helpful AI assistant for Digitpen Hub.' }, { role: 'user', content: prompt.trim() }], opts: { maxTokens: 800, temperature: 0.7 } });
