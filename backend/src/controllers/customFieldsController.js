...
async function setFieldDependencies(req, res) {
  ...
  try {
    ...
    const { validateDependency } = require('../utils/fieldDependencyEvaluator');

    // Validate each dependency rule using the new validator
    ...
    await logFieldUpdate(fieldId, orgId, req.user.id, {
      ...
      has_advanced_logic: dependencies.some(d => d.logic_operator || d.conditions)
    });
    ...
    res.json({
      ...
      advanced_features: dependencies.some(d => d.logic_operator || d.conditions)
    });
  } catch (error) {
