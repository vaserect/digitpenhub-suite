...
async function getEntry(req, res) {
  ...
  const entry = { ...rows[0], password: decrypt(rows[0].password) };
  res.json({ entry });
}
...
async function createEntry(req, res) {
  ...
  const { rows } = await db.query(
    `INSERT INTO password_entries (org_id,title,category,username,password,strength,url,notes,tags) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id,org_id,title,category,username,url,notes,tags,strength,last_used,created_at,updated_at`,
    [req.user.orgId, title.trim(), category||'General', username||null, encrypt(password), computeStrength(password), url||null, notes||null, tags||[]]
  );
...
async function updateEntry(req, res) {
  ...
  if (password !==undefined){updates.push(`password=$${i++}`); vals.push(encrypt(password)); updates.push(`strength=$${i++}`); vals.push(computeStrength(password));}
  ...
  const { rows } = await db.query(
    `UPDATE password_entries SET ${updates.join(',')} WHERE id=$${i} AND org_id=$${i+1} RETURNING id,org_id,title,category,username,url,notes,tags,strength,last_used,created_at,updated_at`,
    vals
  );
  if (!rows.length) return res.status(404).json({ error: 'Not found.' });
