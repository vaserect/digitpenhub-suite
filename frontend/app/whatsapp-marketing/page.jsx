...
export default function WhatsappMarketingPage() {
  ...
  useEffect(() => {
    apiFetch('/api/v1/whatsapp/contacts').then(d => setData(d.contacts || [])).catch(() => toast.error('Failed to load')).finally(() => setLoading(false));
  }, []);
  async function handleCreate(e) { e.preventDefault(); try { await apiFetch('/api/v1/whatsapp/contacts', {method:'POST',body:JSON.stringify(form)}); toast.success('Added!'); setShowForm(false); setForm({name:'',phone:''}); const d=await apiFetch('/api/v1/whatsapp/contacts'); setData(d.contacts||[]); } catch(err) { toast.error(err.message); } }
  return (
    <ModulePage back={{label:'Workspace',onClick:()=>router.push('/')}} title="WhatsApp Marketing" description="Reach customers via WhatsApp campaigns.">
