...
export default function EmptyState({ icon = 'inbox', title, description, action, compact }) {
  const IconSvg = typeof icon === 'string' && ICONS[icon] ? ICONS[icon] : typeof icon === 'string' ? null : icon;
  return (
    <div className={['empty-state-pro', compact ? 'is-compact' : ''].filter(Boolean).join(' ')}>
      <div className="es-icon" aria-hidden="true">
        {IconSvg || <span className="es-icon-fallback">{icon}</span>}
      </div>
