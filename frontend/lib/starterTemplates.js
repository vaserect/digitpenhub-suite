...
export function getLeadFormStarterTemplates() {
  return [
    ...
    },
    {
      ...
      description: 'A minimal email capture form for blog subscribers and content marketing.',
      highlights: ['Email only', 'Interest selection', 'GDPR consent checkbox'],
      draft: {
        name: 'Newsletter Signup',
        thankYouMessage: 'Thanks for subscribing! Check your inbox for a confirmation email.',
        fields: [
          leadField('email_address', 'email', 'Email address', 'your@email.com', { required: true }),
          leadField('interests', 'select', 'What interests you most?', '', { required: false, options: ['Product updates', 'Industry news', 'Tips & tutorials', 'Case studies'] }),
          leadField('consent', 'checkbox', '', 'I agree to receive marketing emails', { required: true }),
        ],
      },
    },
    {
      ...
      description: 'A structured application form for recruiting and hiring workflows.',
      highlights: ['Resume upload', 'Salary expectations', 'Availability date'],
      draft: {
        name: 'Job Application Form',
        thankYouMessage: 'Thanks for applying! We will review your application and get back to you soon.',
        fields: [
          leadField('full_name', 'text', 'Full name', 'Jane Doe', { required: true }),
          leadField('email_address', 'email', 'Email address', 'jane@example.com', { required: true }),
          leadField('phone_number', 'phone', 'Phone number', '+234...', { required: true }),
          leadField('position_applied', 'text', 'Position applied for', 'e.g., Senior Developer', { required: true }),
          leadField('linkedin_url', 'text', 'LinkedIn profile', 'https://linkedin.com/in/...', { required: false }),
          leadField('years_experience', 'select', 'Years of experience', '', { required: true, options: ['0-2 years', '3-5 years', '6-10 years', '10+ years'] }),
          leadField('salary_expectations', 'text', 'Salary expectations', 'e.g., ₦500,000/month', { required: false }),
          leadField('cover_letter', 'textarea', 'Why are you a good fit?', 'Tell us about your experience and motivation...', { required: true }),
        ],
      },
    },
    {
      ...
      description: 'A help desk intake form for customer support and technical issues.',
      highlights: ['Priority selection', 'Issue category', 'Account details'],
      draft: {
        name: 'Support Ticket Form',
        thankYouMessage: 'Your ticket has been submitted. Our support team will respond within 24 hours.',
        fields: [
          leadField('full_name', 'text', 'Full name', 'Jane Doe', { required: true }),
          leadField('email_address', 'email', 'Email address', 'jane@example.com', { required: true }),
          leadField('account_id', 'text', 'Account ID or username', '', { required: false }),
          leadField('issue_category', 'select', 'Issue category', '', { required: true, options: ['Technical issue', 'Billing question', 'Feature request', 'Account access', 'Other'] }),
          leadField('priority', 'select', 'Priority', '', { required: true, options: ['Low', 'Medium', 'High', 'Urgent'] }),
          leadField('issue_description', 'textarea', 'Describe the issue', 'Please provide as much detail as possible...', { required: true }),
        ],
      },
    },
    {
      ...
      description: 'A B2B form for potential partners, affiliates, or strategic alliances.',
      highlights: ['Company details', 'Partnership type', 'Revenue/traffic info'],
      draft: {
        name: 'Partnership Inquiry Form',
        thankYouMessage: 'Thanks for your interest in partnering with us. We will review and respond within 3 business days.',
        fields: [
          leadField('contact_name', 'text', 'Contact name', 'Jane Doe', { required: true }),
          leadField('company_name', 'text', 'Company name', 'Acme Corp', { required: true }),
          leadField('email_address', 'email', 'Business email', 'jane@company.com', { required: true }),
          leadField('website', 'text', 'Company website', 'https://example.com', { required: false }),
          leadField('partnership_type', 'select', 'Partnership type', '', { required: true, options: ['Affiliate/Reseller', 'Technology integration', 'Co-marketing', 'Strategic alliance', 'Other'] }),
          leadField('company_size', 'select', 'Company size', '', { required: false, options: ['1-10', '11-50', '51-200', '201-1000', '1000+'] }),
          leadField('partnership_details', 'textarea', 'Partnership proposal', 'Describe how you envision working together...', { required: true }),
        ],
      },
    },
    {
      ...
      description: 'A registration form for online events, webinars, and virtual workshops.',
      highlights: ['Job title capture', 'Company info', 'Question submission'],
      draft: {
        name: 'Webinar Registration',
        thankYouMessage: 'You are registered! We will send the webinar link to your email 1 hour before the event.',
        fields: [
          leadField('full_name', 'text', 'Full name', 'Jane Doe', { required: true }),
          leadField('email_address', 'email', 'Email address', 'jane@example.com', { required: true }),
          leadField('job_title', 'text', 'Job title', 'e.g., Marketing Manager', { required: false }),
          leadField('company_name', 'text', 'Company name', '', { required: false }),
          leadField('questions', 'textarea', 'Questions for the speaker', 'What would you like to learn?', { required: false }),
        ],
      },
    },
    {
      ...
      description: 'A SaaS trial signup form with qualification questions.',
      highlights: ['Use case capture', 'Team size', 'No credit card required message'],
      draft: {
        name: 'Start Your Free Trial',
        thankYouMessage: 'Your trial account is ready! Check your email for login instructions.',
        fields: [
          leadField('full_name', 'text', 'Full name', 'Jane Doe', { required: true }),
          leadField('work_email', 'email', 'Work email', 'jane@company.com', { required: true }),
          leadField('company_name', 'text', 'Company name', 'Acme Corp', { required: true }),
          leadField('team_size', 'select', 'Team size', '', { required: true, options: ['Just me', '2-10', '11-50', '51-200', '200+'] }),
          leadField('use_case', 'select', 'Primary use case', '', { required: true, options: ['Marketing', 'Sales', 'Customer support', 'Project management', 'Other'] }),
          leadField('password', 'text', 'Create password', '', { required: true }),
        ],
      },
    },
    {
      ...
      description: 'A real estate lead form for property viewings and rental inquiries.',
      highlights: ['Property reference', 'Budget range', 'Viewing preference'],
      draft: {
        name: 'Property Inquiry Form',
        thankYouMessage: 'Thanks for your interest. An agent will contact you within 24 hours to schedule a viewing.',
        fields: [
          leadField('full_name', 'text', 'Full name', 'Jane Doe', { required: true }),
          leadField('email_address', 'email', 'Email address', 'jane@example.com', { required: true }),
          leadField('phone_number', 'phone', 'Phone number', '+234...', { required: true }),
          leadField('property_reference', 'text', 'Property reference', 'e.g., PROP-12345', { required: false }),
          leadField('budget_range', 'select', 'Budget range', '', { required: true, options: ['Under ₦500k/month', '₦500k - ₦1M/month', '₦1M - ₦2M/month', 'Above ₦2M/month'] }),
          leadField('viewing_preference', 'select', 'Preferred viewing time', '', { required: false, options: ['Weekday morning', 'Weekday afternoon', 'Weekend morning', 'Weekend afternoon'] }),
          leadField('additional_requirements', 'textarea', 'Additional requirements', 'Any specific needs or questions?', { required: false }),
        ],
      },
    },
  ];
